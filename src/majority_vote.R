...
pathA <- "./TADs/juicer_dmel_TADs_5kb.bed"
pathH <- "./TADs/dmel_TAD_5kb_domains.bed" # *domains.bed
...
#read_tsv("Juicer/OLD/Dmel/all_resolutions/10000_blocks") %>%
#"C:/Users/dival/Documents/Tesis/Juicer/Dmel/dmel_10kb/10000_blocks"

A <- read_tsv(pathA) %>%
          select(chr1, x1, x2) %>% 
          rename("chr" = chr1,"start" = x1, "end" = x2)

#"./TADs/dmel_TAD_10kb_domains.bed"
# H: HiCExplorer
H <- read.csv(pathH,
              header = FALSE, sep = "\t") %>%
              select(V1, V2, V3) %>%
